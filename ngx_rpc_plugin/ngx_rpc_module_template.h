...
static const char * NGX_RPC_SET_SERVER_METHOD_START[] = {
    ...
    "    if( cf->args->nelts > 2)",
    "    {",
    "        $PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log_file.data = (u_char*)ngx_palloc(cf->pool, 256);",
    "        $PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log_file.len  =",
    "            snprintf((char*)$PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log_file.data, 256, \"logs/%s\", value[2].data);",
    "",
    "        $PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log  = (ngx_log_t*)ngx_palloc(cf->pool, sizeof(ngx_log_t));",
    "        memset($PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log, 0, sizeof(ngx_log_t));",
    "",
    "        $PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log->file = ",
                  "ngx_conf_open_file(cf->cycle, &$PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log_file);",
    "        $PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log->action = (char*)\"/$PROTO_NAMEPATH$/$PROTO_SERVER_NAME_LOWER$\";",
    "        $PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log->log_level = NGX_LOG_DEBUG;",
    "    }",
    ""
};
...
static const char * NGX_RPC_SET_SERVER_METHOD_ITEM[] = {
    ...
    "        method->name    = ngx_string(\"/$PROTO_NAMEPATH$/$PROTO_SERVER_NAME_LOWER$/$PROTO_SERVER_METHOD_NAME$\");",
    ...
    "    {",
    "        method_conf_t *method = (method_conf_t *)ngx_array_push(rpc_conf->method_array);",
    "        method->name    = ngx_string(\"/$PROTO_NAMEPATH_DOT$.$PROTO_SERVER_NAME_LOWER$.$PROTO_SERVER_METHOD_NAME$\");",
    "        method->_impl   = $PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_impl;",
    "        method->handler = ngxrpc_$PROTO_NAME$_$PROTO_SERVER_NAME_LOWER$_$PROTO_SERVER_METHOD_NAME$;",
    "        method->log     = $PROTO_NAME$_conf->$PROTO_SERVER_NAME_LOWER$_log;",
    "        method->exec_in_nginx = 1;",
